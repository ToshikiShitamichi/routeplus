...
import { fetchStudents, fetchStudentProgress, createInvitation, fetchInvitations } from '../../api/admin';
...
export default function AdminDashboard() {
    ...
    useEffect(() => {
        ...
        fetchStudents()
            ...
            .catch((err) => console.error('生徒取得エラー', err))
            .finally(checkDone);

        fetchInvitations()
            .then(setInvitations)
            .catch((err) => console.error('招待取得エラー', err))
            .finally(checkDone);
    }, []);
        ...
        <div className={styles.shell}>
            <aside className={styles.sidebar}>
                ...
                <nav className={styles.nav}>
                    <div
                        className={`${styles.navItem} ${tab === 'students' ? styles.active : ''}`}
                        onClick={() => { setTab('students'); setSelectedStudent(null); }}
                    >
                        生徒一覧
                    </div>
                    <div
                        className={`${styles.navItem} ${tab === 'invitations' ? styles.active : ''}`}
            ...
            <main className={styles.main}>
                ...
                {!loading && tab === 'students' && !selectedStudent && (
                    <section className={styles.section}>
                        {students.length === 0 && (
                            <p className={styles.empty}>まだ生徒がいません</p>
                        )}
                        {students.map((group) => (
                                            ...
                                            {group.tasks.map((task) => (
                                                <li key={task.id} className={styles.taskItem}>
                                                    ...
                                                    <div className={styles.taskRight}>
                                                        {task.github_url && (
                                                            <a href={task.github_url} target="_blank" rel="noopener noreferrer" className={styles.taskLink}>
                                                                GitHub
                                                            </a>
                                                        )}
                        ...
                        <div className={styles.inviteForm}>
                            ...
                            {inviteResult && (
                                <div className={styles.inviteResult}>
                                    ...
                                    <button className={styles.copyButton} onClick={() => handleCopy(inviteResult.invite_url)}>
